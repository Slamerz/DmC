...
using EvilMortyBot.Modules.CustomReactions.Services;
using System;
...
namespace EvilMortyBot.Modules.CustomReactions.Extensions
{
    public static class Extensions
    {
        private static readonly Regex imgRegex = new Regex("%(img|image):(?<tag>.*?)%", RegexOptions.Compiled);

        private static readonly EvilMortyRandom rng = new EvilMortyRandom();
        ...
        public static Dictionary<Regex, Func<Match, Task<string>>> regexPlaceholders = new Dictionary<Regex, Func<Match, Task<string>>>()
        {
            { imgRegex, async (match) => {
                ...
                var img = (elems.ElementAtOrDefault(new EvilMortyRandom().Next(0, elems.Length))?.Children?.FirstOrDefault() as IHtmlImageElement);
