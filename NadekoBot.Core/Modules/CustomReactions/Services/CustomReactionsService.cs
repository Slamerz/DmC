...
using EvilMortyBot.Modules.CustomReactions.Extensions;
using EvilMortyBot.Modules.Permissions.Common;
using EvilMortyBot.Modules.Permissions.Services;
using EvilMortyBot.Core.Services.Impl;
using Newtonsoft.Json;

namespace EvilMortyBot.Modules.CustomReactions.Services
{
    public class CustomReactionsService : IEarlyBlockingExecutor, INService
    {
        ...
        public CustomReactionsService(PermissionService perms, DbService db, EvilMortyStrings strings,
            DiscordSocketClient client, CommandHandler cmd, IBotConfigProvider bc, IUnitOfWork uow,
            IDataCache cache, GlobalPermissionService gperm, EvilMortyBot bot)
        {
        ...
        public CustomReaction TryGetCustomReaction(IUserMessage umsg)
        {
            ...
            if (GuildReactions.TryGetValue(channel.Guild.Id, out CustomReaction[] reactions))
                if (reactions != null && reactions.Any())
                {
                    ...
                    if (rs.Length != 0)
                    {
                        var reaction = rs[new EvilMortyRandom().Next(0, rs.Length)];
                        if (reaction != null)
                        {
            ...
            if (grs.Length == 0)
                return null;
            var greaction = grs[new EvilMortyRandom().Next(0, grs.Length)];
