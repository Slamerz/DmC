...
using EvilMortyBot.Modules.Pokemon.Services;

namespace EvilMortyBot.Modules.Pokemon
{
    public class Pokemon : EvilMortyTopLevelModule<PokemonService>
    {
            ...
            if (targetType == null)
            {
                await Context.Channel.EmbedAsync(_service.PokemonTypes.Aggregate(new EmbedBuilder().WithDescription("List of the available types:"),
                        (eb, pt) => eb.AddField(efb => efb.WithName(pt.Name)
                                                          .WithValue(pt.Icon)
                                                          .WithIsInline(true)))
                            .WithColor(EvilMortyBot.OkColor)).ConfigureAwait(false);
                return;
            }
