...
using EvilMortyBot.Modules.Music.Common.SongResolver;
using EvilMortyBot.Common.Collections;
using System;
using Microsoft.EntityFrameworkCore;

namespace EvilMortyBot.Modules.Music.Services
{
    public class MusicService : INService, IUnloadableService
    {
        ...
        public MusicService(DiscordSocketClient client, IGoogleApiService google,
            EvilMortyStrings strings, ILocalization localization, DbService db,
            SoundCloudApiService sc, IBotCredentials creds, EvilMortyBot bot)
        {
            _client = client;
            _google = google;
            _strings = strings;
        ...
        public async Task TryQueueRelatedSongAsync(SongInfo song, ITextChannel txtCh, IVoiceChannel vch)
        {
            ...
            var si = await ResolveSong(related[new EvilMortyRandom().Next(related.Length)], _client.CurrentUser.ToString(), MusicType.YouTube);
            if (si == null)
