...
using EvilMortyBot.Modules.Searches.Common.Exceptions;
using Newtonsoft.Json;
using NLog;

namespace EvilMortyBot.Modules.Searches.Services
{
    public class StreamNotificationService : INService
    {
        ...
        public StreamNotificationService(EvilMortyBot bot, DbService db, DiscordSocketClient client,
            EvilMortyStrings strings, IDataCache cache, IBotCredentials creds)
        {
            _db = db;
            _client = client;
            _strings = strings;
        ...
        public EmbedBuilder GetEmbed(FollowedStream fs, IStreamResponse status, ulong guildId)
        {
            var embed = new EmbedBuilder()
                ...
                .AddField(efb => efb.WithName(GetText(fs, "viewers"))
                                ...
                                .WithIsInline(true))
                .WithColor(status.Live ? EvilMortyBot.OkColor : EvilMortyBot.ErrorColor);
