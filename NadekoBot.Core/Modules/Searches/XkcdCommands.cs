...
namespace EvilMortyBot.Modules.Searches
{
    ...
    {
        [Group]
        public class XkcdCommands : EvilMortySubmodule
        {
            ...
            public async Task Xkcd(string arg = null)
            {
                if (arg?.ToLowerInvariant().Trim() == "latest")
                {
                    using (var http = new HttpClient())
                    {
                        ...
                        var embed = new EmbedBuilder().WithColor(EvilMortyBot.OkColor)
                ...
                await Xkcd(new EvilMortyRandom().Next(1, 1750)).ConfigureAwait(false);
            }
