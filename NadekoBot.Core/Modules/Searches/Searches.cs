...
{
    public partial class Searches : EvilMortyTopLevelModule<SearchesService>
    {
        ...
        public async Task RandomImage([Remainder] string terms = null)
        {
            ...
            try
            {
                var res = await _google.GetImageAsync(terms, new EvilMortyRandom().Next(0, 50)).ConfigureAwait(false);
                var embed = new EmbedBuilder()
                    .WithOkColor()
                    .WithAuthor(eab => eab.WithName(GetText("image_search_for") + " " + terms.TrimTo(50))
                        .WithUrl("https://www.google.rs/search?q=" + terms + "&source=lnms&tbm=isch")
            ...
            catch
            {
                ...
                var img = (elems.ElementAtOrDefault(new EvilMortyRandom().Next(0, elems.Count))?.Children?.FirstOrDefault() as IHtmlImageElement);
        ...
        public async Task Shorten([Remainder] string arg)
        {
            ...
            await Context.Channel.EmbedAsync(new EmbedBuilder().WithColor(EvilMortyBot.OkColor)
        ...
        public async Task MagicTheGathering([Remainder] string name)
        {
            ...
            using (var http = new HttpClient())
            {
                ...
                try
                {
                    ...
                    var item = items[new EvilMortyRandom().Next(0, items.Length)];
