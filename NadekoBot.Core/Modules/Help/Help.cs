...
using EvilMortyBot.Modules.Help.Services;
using EvilMortyBot.Modules.Permissions.Services;
using EvilMortyBot.Common;
using EvilMortyBot.Common.Replacements;
using Newtonsoft.Json;

namespace EvilMortyBot.Modules.Help
{
    public class Help : EvilMortyTopLevelModule<HelpService>
    {
        public const string PatreonUrl = "https://patreon.com/evilMortybot";
        ...
        public async Task Hgit()
        {
            ...
            foreach (var com in _cmds.Commands.OrderBy(com => com.Module.GetTopLevelModule().Name).GroupBy(c => c.Aliases.First()).Select(g => g.First()))
            {
                ...
                var opt = ((EvilMortyOptions)com.Attributes.FirstOrDefault(x => x is EvilMortyOptions))?.OptionType;
                if (opt != null)
                {
        ...
        public async Task Guide()
        {
            await ConfirmLocalized("guide", 
                "https://evilMortybot.me/commands",
                "http://evilMortybot.readthedocs.io/en/latest/").ConfigureAwait(false);
        }
