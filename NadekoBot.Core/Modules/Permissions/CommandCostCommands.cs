...
namespace EvilMortyBot.Modules.Permissions
{
    ...
    {
        [Group]
        public class CommandCostCommands : EvilMortySubmodule
        {
            ...
            {
                //_commandCosts = new ConcurrentDictionary<string, int>(EvilMortyBot.BotConfig.CommandCosts.ToDictionary(
            ...
            //    else
            //        await Context.Channel.SendConfirmAsync($"{Format.Bold(cmd.Name)} now costs {cost}{EvilMortyBot.BotConfig.CurrencySign} to run.").ConfigureAwait(false);
            //}
        }
    }
}
