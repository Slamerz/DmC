...
using EvilMortyBot.Modules.NSFW.Exceptions;
using EvilMortyBot.Modules.Searches.Exceptions;

namespace EvilMortyBot.Modules.NSFW
{
    // thanks to halitalf for adding autoboob and autobutt features :D
    public class NSFW : EvilMortyTopLevelModule<SearchesService>
    {
        ...
        private async Task InternalHentai(IMessageChannel channel, string tag, bool noError)
        {
            ...
            var type = (DapiSearchType)arr.GetValue(new EvilMortyRandom().Next(2, arr.Length));
        ...
        private async Task InternalBoobs(IMessageChannel Channel)
        {
            ...
            {
                JToken obj;
                obj = JArray.Parse(await _service.Http.GetStringAsync($"http://api.oboobs.ru/boobs/{new EvilMortyRandom().Next(0, 10330)}").ConfigureAwait(false))[0];
        ...
        private async Task InternalButts(IMessageChannel Channel)
        {
            ...
            {
                JToken obj;
                obj = JArray.Parse(await _service.Http.GetStringAsync($"http://api.obutts.ru/butts/{new EvilMortyRandom().Next(0, 4335)}").ConfigureAwait(false))[0];
            ...
            t = new Timer(async (state) =>
            {
                try
                {
                    if (tagsArr == null || tagsArr.Length == 0)
                        ...
                        await InternalHentai(Context.Channel, tagsArr[new EvilMortyRandom().Next(0, tagsArr.Length)], true).ConfigureAwait(false);
                }
                catch
                {
                    // ignored
