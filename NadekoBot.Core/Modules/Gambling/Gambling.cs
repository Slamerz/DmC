...
using System;
using EvilMortyBot.Modules.Gambling.Services;
...
namespace EvilMortyBot.Modules.Gambling
{
    public partial class Gambling : GamblingTopLevelModule<GamblingService>
    {
        ...
        public async Task Raffle([Remainder] IRole role = null)
        {
            ...
            var usr = membersArray[new EvilMortyRandom().Next(0, membersArray.Length)];
...
";
        ...
        //        var generator = new EvilMortyRandom();
        ...
        private async Task InternallBetroll(long amount)
        {
            ...
            var rnd = new EvilMortyRandom().Next(0, 101);
        ...
        public async Task Rps(RpsPick pick, ShmartNumber amount = default)
        {
            ...
            var evilMortyPick = (RpsPick)new EvilMortyRandom().Next(0, 3);
            ...
            else if ((pick == RpsPick.Paper && evilMortyPick == RpsPick.Rock) ||
                     (pick == RpsPick.Rock && evilMortyPick == RpsPick.Scissors) ||
                     (pick == RpsPick.Scissors && evilMortyPick == RpsPick.Paper))
            {
                ...
                msg = GetText("rps_win", Context.User.Mention,
                    getRpsPick(pick), getRpsPick(evilMortyPick));
            }
            else
            {
                embed.WithErrorColor();
                amount = 0;
                msg = GetText("rps_win", Context.Client.CurrentUser.Mention, getRpsPick(evilMortyPick),
                    getRpsPick(pick));
            }
