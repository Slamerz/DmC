...
using EvilMortyBot.Modules.Gambling.Common;
using Discord;
            ...
            try
            {
                using (var req = new HttpRequestMessage(HttpMethod.Get, _creds.VotesUrl))
                {
                    ...
                    var ids = JsonConvert.DeserializeObject<VoteModel[]>(resStr)
                        .Select(x => x.User)
                        .Distinct();
                    await _cs.AddBulkAsync(ids, ids.Select(x => "Voted - <https://discordbots.org/bot/evilMorty/vote>"), ids.Select(x => 10L), true);
                }
