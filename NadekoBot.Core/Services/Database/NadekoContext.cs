...
namespace EvilMortyBot.Core.Services.Database
{
    public class EvilMortyContextFactory : IDesignTimeDbContextFactory<EvilMortyContext>
    {        
        public EvilMortyContext CreateDbContext(string[] args)
        {
            var optionsBuilder = new DbContextOptionsBuilder<EvilMortyContext>();
            var builder = new SqliteConnectionStringBuilder("Data Source=data/EvilMortyBot.db");
            builder.DataSource = Path.Combine(AppContext.BaseDirectory, builder.DataSource);
            optionsBuilder.UseSqlite(builder.ToString());
            var ctx = new EvilMortyContext(optionsBuilder.Options);
            ctx.Database.SetCommandTimeout(60);
            return ctx;
        }
    }

    public class EvilMortyContext : DbContext
    {
        ...
        public EvilMortyContext(DbContextOptions<EvilMortyContext> options) : base(options)
        {
        }
