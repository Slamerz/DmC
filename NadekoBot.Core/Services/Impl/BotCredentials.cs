    ...
    public class BotCredentials : IBotCredentials
    {
        ...
        public BotCredentials()
        {
            ...
            if (!File.Exists(_credsFileName))
                _log.Warn($"credentials.json is missing. Attempting to load creds from environment variables prefixed with 'EvilMortyBot_'. Example is in {Path.GetFullPath("./credentials_example.json")}");
            try
            {
                var configBuilder = new ConfigurationBuilder();
                configBuilder.AddJsonFile(_credsFileName, true)
                    .AddEnvironmentVariables("EvilMortyBot_");
                ...
                {
                    if (string.IsNullOrWhiteSpace(ShardRunCommand))
                        ShardRunCommand = "EvilMortyBot.exe";
                    if (string.IsNullOrWhiteSpace(ShardRunArguments))
                ...
                if (string.IsNullOrWhiteSpace(portStr))
                    ShardRunPort = new EvilMortyRandom().Next(5000, 6000);
                else
                ...
                Db = new DBConfig(string.IsNullOrWhiteSpace(dbSection["Type"])
                            ...
                            string.IsNullOrWhiteSpace(dbSection["ConnectionString"])
                                ? "Data Source=data/EvilMortyBot.db"
        ...
        private class CredentialsModel
        {
            ...
            public DBConfig Db { get; set; } = new DBConfig("sqlite", "Data Source=data/EvilMortyBot.db");
