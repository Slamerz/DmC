...
namespace EvilMortyBot.Core.Services.Impl
{
    public class SoundCloudApiService : INService
    {
        ...
        public async Task<SoundCloudVideo> ResolveVideoAsync(string url)
        {
            ...
            {
                response = await http.GetStringAsync($"https://scapi.evilMortybot.me/resolve?url={url}").ConfigureAwait(false);
            }
        ...
        public async Task<SoundCloudVideo> GetVideoByQueryAsync(string query)
        {
            ...
            using (var http = new HttpClient())
            {
                response = await http.GetStringAsync($"https://scapi.evilMortybot.me/tracks?q={Uri.EscapeDataString(query)}").ConfigureAwait(false);
            }
    ...
    public class SoundCloudVideo
    {
        ...
        public async Task<string> StreamLink()
        {
            using (var http = new HttpClient())
            {
                return await http.GetStringAsync($"http://scapi.evilMortybot.me/stream/{Id}");
            }
        }
    }
